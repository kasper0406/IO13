...
This section presents implementation details on how the heap has
...
\begin{itemize}
  ...
  \item $M$ is the amount of memory available.
\end{itemize}
...
that each block is associated with a node in the internal tree.
...
During sifting, the elements of the current node and its parent are merged.
...
\]
elements to the child. But then it might be that the parent gets

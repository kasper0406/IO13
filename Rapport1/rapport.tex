...
2GB of elements\todo{Ikke sandt, det er kun 1GB elementer vi har testet paa. Ellers ville MMap read ogsaa vaere meget maerkelige! Maaske vi for Mmap skulle lave en 2GB test}. This choice was made to make sure the data was
...
performance, we only did the measurements displayed in
Table~\ref{tbl:poor-streams}\todo{Comment on these!}.

\begin{table}[h!]
  \centering
  \begin{tabular}{c|cc}
    $k$ & Read time & Write time \\
    \hline
    1   & 129.752s  & 649.089s   \\
    4   & 130.710s  & -          \\
    16  & 135.268   & -          \\
    64  & 184.103   & -          \\
  \end{tabular}
  \caption{Running times for the streams using \texttt{read} and
    \texttt{write} system calls directly.}
\end{table}

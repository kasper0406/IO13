...
tested the streams in respectively reading/writing $n$ elements from/to a file. Each test uses $k$ interleaved streams to do this, such
...
the input and output streams performed very similar, hence the absence\todo{er det fsync naar vi bruger write/read og ikke fread/fwrite?}
of effect in buffer size change showed in
Figure~\ref{fig:buffered-output} can be explained by the operating
system delaying writes to the disk.\todo{Maaske mere sandsynligt at det er disken der goer det?}
...
move its read heads\todo{Other reasons? Ved ikke helt om jeg koeber den. Med det argument burde der vel ikke vaere meget forskel paa 4 og 16? Et andet argument kunne vaere at det bliver mere tilfaeldigt hvad der bliver laest naeste gang}.
...
512B\todo{Check this. Vi har mere data om dette nu. I buffered_input_xxxsmall.}. If smaller amount should be written, it is

...
\texttt{\url{https://github.com/kasper0406/IO13/tree/master/Project1/}}
\begin{description}
\item[/] Contains the implementations of the different streams and
...
streams were performed, and contains an analysis of their performance. First each stream will
...
In the merge step of the sorting application, the input streams are
...
when they are too big to be in memory. Comparing with buffered input streams we see a large decrease in performance earlier with buffered write. We believe it is due to a large write buffer in the operating system to facilitate asynchronous write. Therefore, less memory is available for the output streams.
...
memory mapped input stream. Except for the case $k = 256$, the plot shows
...
increasing $k$. However, for $k=256$ which we do not fully understand but we believe it is due to the synchronization algorithm underlying memory mapped files.
...
the disk is a sector, which on the test machine is 512B.  We have made experiments that indicates writing to disk is delayed until 512B have been
written, while for reading we have read-ahead which is larger than 512B (128kB), which can explain the
...
We choose the 2MB buffered output stream since we also choose the corresponding buffered input stream. As indicated by Figure~\ref{fig:best-output} it might have been a better choice to pick a smaller buffer size. Due to time constraints we did not experiments with a smaller buffer size.
...
32$ and $M = 64\textrm{MB}, k = 64$ to be the ones using less memory, both using 96MB and simultaneously with the minimum tree height.
